...
namespace TBSGame.MapHandler 
{
    struct TileInfo
    {
        const string ERROR_MESSAGE = "Error in reading tiles.txt of selected map";
        ...
        {
            var tmp = line.Split(';');
            if (tmp.Length != 5) throw new Exception(ERROR_MESSAGE);
            ...
            if (double.TryParse(tmp[2], out tmpArmorBonus)) ArmorBonus = tmpArmorBonus;
            else throw new Exception(ERROR_MESSAGE);
